...
void ConnectionImpl::noDelay(bool enable) {
  ...
  int rc = os_sys_calls.getsockname(fd(), &addr, &len);
  RELEASE_ASSERT(rc == 0);
  ...
  rc = os_sys_calls.setsockopt(fd(), IPPROTO_TCP, TCP_NODELAY, &new_value, sizeof(new_value));
#ifdef __APPLE__
...
void ConnectionImpl::onWriteReady() {
  ...
  if (connecting_) {
    ...
    Api::OsSysCalls& os_sys_calls = Api::OsSysCallsSingleton::get();
    int rc = os_sys_calls.getsockopt(fd(), SOL_SOCKET, SO_ERROR, &error, &error_size);
    ASSERT(0 == rc);
